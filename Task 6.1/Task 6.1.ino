...
void loop() {
  ...
  float angularVelocity = yawAngle /131;// conversion to degrees per second (number may vary depending on the sensor)(check datasheet)
                                  // datasheet page 31 since this sensor has a range of +-250 

  float IntegrationDtheta = angularVelocity * Time;

  totalAngle += IntegrationDtheta;
  
  Serial.println(totalAngle);  // Print the yaw angle 
